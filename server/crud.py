...
def get_classification_counts(db: Session):
    total_good = db.query(app_models.ClassificationResult).filter(app_models.ClassificationResult.class_name == 'GOOD').count()
    total_no_good = db.query(app_models.ClassificationResult).filter(app_models.ClassificationResult.class_name == 'NO GOOD').count()
